...
const toB64Url = b64 => b64.replace(/=+/g, "").replace(/\+/g, "-").replace(/\//g, "_");
const fromB64Url = b64u => b64u.replace(/-/g, "+").replace(/_/g, "/");

devek.stringToBase64 = value =>
  !value ? emptyB64 : btoa(String.fromCharCode.apply(null, encoder.encode(value)));
...
devek.formatArray = (array, format) => {
  const f = format.toLowerCase();
  if (f === "base64") return devek.arrayToBase64(array);
  if (f === "utf-8") return devek.arrayToUTF8(array);
  if (f === "hex") return devek.arrayToHexString(array);
...
devek.arrayToHexString = (a, j = "", splitAt = 0) => {
  const result = (Array.isArray(a) ? a : [...a]).map(x => x.toString(16).padStart(2, "0")).join(j);
  return splitAt > 0 ? result.match(new RegExp(`.{1,${splitAt}}`, "g")).join("\n") : result;
};
