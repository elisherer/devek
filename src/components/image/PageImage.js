			...
			<ToolbarSection {...dropHandlers}>
				...
				{type === "filters" && (
					<Toolbar loaded={loaded}>
						...
						<button
							className="tool"
							disabled={disabled}
							onClick={actions.grayscale}
							title="Grayscale"
						>
							<Icon path={mdiGradient} size={1} />
						</button>
						<button
							className="tool"
							disabled={disabled}
							onClick={actions.sepia}
							title="Sepia"
						>
				...
				{type === "picker" && (
					<Toolbar loaded={loaded}>
						<div>
							Picker:{" "}
							<input disabled={disabled} type="color" readOnly value={color} />{" "}
							►{" "}
				...
				{type === "crop" && (
					<Hint>
						* You can draw a rectangle on the image to set the coordinates
						before cropping
					</Hint>
				)}
			</ToolbarSection>
			...
			<CanvasWrapper {...dropHandlers}>
				...
				{src && loaded && (
					<img
						...
						draggable={false}
						onMouseUp={
							picker ? actions.pick : cropper ? onCropMouseUp : undefined
						}
						onMouseMove={
							picker ? actions.peek : cropper ? onCropMouseMove : undefined
						}
