...
const PageData = () => {
	...
	const handleImport = useCallback(() => {
		...
		try {
			...
			if (
				importData.some(
					x =>
						!data.actions[x.action] ||
						(data.actions[x.action].parameters && !x.parameters)
				)
			)
	...
	return (
		<div>
			<p>
				Enter your data in raw format to the source text box, pick the actions
				to process with their required parameters, order and run.
			</p>
			<span>Source:</span>
			<TextArea
				autoFocus
			...
			{output && (
				<>
					<h1>Result</h1>
					<span>Output:</span>
					<CopyToClipboard from="data_output" />{" "}
