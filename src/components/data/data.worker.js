...
const actions = {
	Split: (input, { By }) =>
		flat(parseArrayOrString(input).map(x => x.split(parseString(By)))),
	Wrap: (input, { Prefix, Suffix }) =>
		parseArrayOrString(input).map(x => Prefix + x + Suffix),
	ChangeCase: (input, { Case }) =>
		parseArrayOrString(input).map(x =>
			x.toString()[Case === "Uppercase" ? "toUpperCase" : "toLowerCase"]()
		),
	...
	Filter: (input, { Pattern, Flags, Remove }) => {
		const regexp = new RegExp(Pattern, Flags.join(""));
		const filter =
			Remove === "Matching" ? x => !regexp.test(x) : x => regexp.test(x);
	...
	Sort: (input, { Type, Order }) =>
		parseArrayOrString(input).sort(sorts[Type + Order]),
...
onmessage = event => {
	...
	const result = pipe.reduce(
		(a, c) => actions[c.action](a, c.parameters),
		input
	);
