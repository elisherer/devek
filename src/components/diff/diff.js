...
function LCS(
	...
	if (lowerA === upperA) {
		...
		while (lowerB < upperB) dataB.modified[lowerB++] = 1;
	} else if (lowerB === upperB) {
		// mark as deleted lines.
		while (lowerA < upperA) dataA.modified[lowerA++] = 1;
	} else {
...
function optimize(data) {
	...
	while (startPos < data.length) {
		while (startPos < data.length && !data.modified[startPos]) startPos++;
		endPos = startPos;
		while (endPos < data.length && data.modified[endPos]) endPos++;
...
function diffText(textA, textB, trimSpace, ignoreSpace, ignoreCase, separator) {
	...
	const dataA = new DiffData(
		diffCodes(a, hash, trimSpace, ignoreSpace, ignoreCase)
	);

	// The B-Version of the data (modified data) to be compared.
	const b = textB.replace(/\r/g, "").split(separator);
	const dataB = new DiffData(
		diffCodes(b, hash, trimSpace, ignoreSpace, ignoreCase)
	);
