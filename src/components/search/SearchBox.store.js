...
const actionCreators = {
    ...
    if (!closeSearchBoxHandler)
      closeSearchBoxHandler = e => {
        if (e.key === "ArrowDown") {
          ...
          actions.search_choose(navigate);
          e.preventDefault();
        }
      };
  ...
  search_choose: navigate => state => {
    if (state.search && (state.paths[state.index] || state.paths.length === 1)) {
      navigate(state.paths[state.index] ? state.paths[state.index].path : state.paths[0].path);
      return initialState;
    }
    return state;
  },
...
export const { actions, useStore } = createStore(actionCreators, initialState, "search");
