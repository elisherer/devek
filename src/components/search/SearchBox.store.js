...
const actionCreators = {
			...
			paths: index
				.filter(
					t =>
						...
						((flatMap[t].keyword &&
							flatMap[t].keyword.toLowerCase().includes(lower)) ||
							(flatMap[t].title &&
								flatMap[t].title.toLowerCase().includes(lower)) ||
							(flatMap[t].description &&
								flatMap[t].description.toLowerCase().includes(lower)))
				)
	...
	search_choose: history => state => {
		if (
			state.search &&
			(state.paths[state.index] || state.paths.length === 1)
		) {
			history.push(
				state.paths[state.index]
					? state.paths[state.index].path
					: state.paths[0].path
			);
			return initialState;
		}
