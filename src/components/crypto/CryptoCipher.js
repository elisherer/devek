...
const CryptoCipher = ({
	...
	<div>
		...
		<Radio
			flexBasis={30}
			options={["AES-CBC", "AES-CTR", "AES-GCM", "RSA-OAEP"]}
			value={alg}
			onClick={actions.cipherAlg}
		/>
		...
		{alg === "RSA-OAEP" ? (
			...
			<>
				...
				<Radio
					flexBasis={30}
					options={["None", "OpenSSL"]}
					value={kdf}
					onClick={actions.cipherKDF}
				/>

				{kdf === "None" && (
					<>
						<label>
							Position of {alg === "AES-CTR" ? "CTR" : "IV"} in cipher:
						</label>
						<Radio
							flexBasis={30}
							options={["Start", "End", "None"]}
							value={position}
							onClick={actions.cipherPosition}
						/>

						<NoteText>
							* You can use the <Link to="/crypto/generate">generate</Link> page
						...
						{alg === "AES-CTR" && (
							<>
								...
								<TextBox
									autoComplete="off"
									onChange={actions.cipherAESCounter}
									value={aesCounter}
									placeholder="Leave blank to generate/extract"
								/>
							</>
						)}
				...
				{kdf === "OpenSSL" && (
					<>
						...
						<Checkbox
							label="Salt: (Hex)"
							checked={useSalt}
							onChange={actions.useSalt}
						/>
						<TextBox
							autoComplete="off"
							disabled={!useSalt}
							onChange={actions.salt}
							value={salt}
							placeholder="Leave blank to generate/extract"
						/>
					</>
			...
			(output.error ? (
				...
				<>
					...
					<Radio
						...
						value={output.format}
						onClick={actions.cipherFormat}
					/>

					<span>Output:</span>
					<CopyToClipboard from="crypto_cipher_out" />
