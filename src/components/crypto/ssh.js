...
const padCount = v => {
  ...
  w.set(v);
  for (let i = v.length; i < w.length; i++) w[i] = i + 1 - v.length;
  return w;
};

const build = (...args) =>
  devek.concatUint8Array.apply(
    null,
    args.reduce((a, c) => {
      if (typeof c === "number") {
    ...
    case "RSA":
      {
        // RSASSA-PKCS1-v1_5
        type = "ssh-rsa";
        array = build(type, bigNum2(jwk.e), bigNum2(jwk.n));
      }
      break;
    case "EC":
      {
...
function privateKeyJWKToSSH(sshpub, jwk) {
  ...
  switch (jwk.kty) {
    case "RSA":
      ...
      break;
    case "EC":
      prv = devek.concatUint8Array(sshpub.decoded, build(bigNum2(jwk.d), sshpub.comment));
      break;
  }
  ...
  const array = devek.concatUint8Array(
    ...
    build(
      ...
      /*# of keys: */ 1,
      sshpub.decoded,
      padCount(devek.concatUint8Array(rnd, rnd, prv)),
    ),
  );
