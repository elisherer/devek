...
const PageCron = ({ location } : { location: Object }) => {
  ...
  const { [mode]: { exp, error, tab, gen } } = state;
  ...
  const domAllValuesFromOne = useMemo(() => tab !== 'day' ? null :
...
, [tab]);
  ...
  const domNamedOptionsFromFirst = useMemo(() => tab !== 'day' ? null :
    Array.from({ length: Cron.config.count[tab+'m'] }).map((x, i) => 
  ...
  const domAllValuesFromZero = useMemo(() => tab !== 'day' ? null :
    Array.from({ length: Cron.config.count[tab+'m'] }).map((x, i) => 
        ...
        {tab === 'day' ? (
          <>
            ...
            <RadioOption id="opt_specific" name={tab} checked={gen[tab].type === 'w,'} 
                        data-type="w," onChange={actions.type}>
              Specific {tab}(s) of the week (multiple selection): <ChecklistBox label={`Choose ${tab} of the week`} options={namedValuesFromFirst} maxShowSelection={10} 
                            data-type="w," value={gen[tab]['w,']} onChange={actions.args}/>
            </RadioOption>
            <RadioOption id="opt_specific_m" name={tab} checked={gen[tab].type === 'm,'} 
                        data-type="m," onChange={actions.type}>
              Specific {tab}(s) of the month  (multiple selection): <ChecklistBox label={`Choose ${tab} of the month`} options={domNamedValuesFromFirst} maxShowSelection={10} 
          ...
          <>       
            ...
            <RadioOption id="opt_specific" name={tab} checked={gen[tab].type === ','} 
                        data-type="," onChange={actions.type}>
              Specific {tab}(s) (multiple selection): <ChecklistBox label={`Choose ${tab}`} options={namedValuesFromFirst} maxShowSelection={10} 
