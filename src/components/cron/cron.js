...
const config = {
	...
	names: {
		...
		[DAY + "m"]: Array.from({ length: 32 }).map(
			(_, i) => i + (ORDINAL[i] || "th")
		)
	}
};
...
const parseValue = (value, index, name, mode) => {
	if (value[0] < 10) return parseInt(value, 10);
	const word = value.toUpperCase();
	if (typeof parseWords?.[mode + name]?.[word] === "number")
		return (
			parseWords[mode + name][word] +
			(mode === CRONTAB && word === "SUN" && index === 1 ? 7 : 0)
		);
...
const parseField = (name, value, mode) => {
	...
	if (EveryNRegExp.test(value)) {
		// 0/0 , */0
		return {
			type: "/",
			"/": value.split("/").map(x => (x === "*" ? 0 : parseInt(x, 10)))
		};
	} else if (name !== DAY && RangeRegExp.test(value)) {
		// range only
		return {
			type: "-",
			"-": value.split("-").map((x, i) => parseValue(x, i, name, mode))
		};
	}
	return {
		type: ",",
		",": value.split(",").reduce((args, arg) => {
			if (arg.includes("-")) {
				const range = arg
					.split("-")
					.map((x, i) => parseValue(x, i, name, mode));
...
const stringifyField = (name, field, mode) => {
	...
	const { type } = field;
	switch (
		type[type.length - 1] // last letter
	) {
		case "*":
			return "*";
		case "/":
			return `${field[type][0]}/${field[type][1]}`;
		case "-":
			return field[type].map(aliaser).join("-");
		default: {
			...
			for (let x of field[type]) {
				...
				output.push(
					aliaser(firstInRange) +
						(lastValue === firstInRange ? "" : "-" + aliaser(lastValue))
				);
				lastValue = x;
				firstInRange = x;
			}
...
const stringifyDayField = (field, mode) => {
	...
	switch (type) {
		case "mLW":
			return "LW";
		case "mL":
			return "L" + (field[type][0] ? "-" + field[type][0] : "");
		case "mW":
			return field[type][0] + "W";
		case "w#":
			return field[type][0] + "#" + field[type][1];
		case "wL":
			return field[type][0] + "L";
	}
...
class Cron {
	...
	static stringify(c, mode) {
		...
		parts.push(
			c.day.type === "*" || c.day.type[0] !== "m"
				? unused
				: stringifyDayField(c.day, mode)
		);
