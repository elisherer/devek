...
import { actions, useStore } from "./PageCron.store";
...
const PageCron = ({ location }) => {
  ...
  return (
    <div>
      ...
      <InputWrapper>
        <TextBox autoFocus selectOnFocus invalid={error} value={exp} onChange={actions.exp} />
      ...
      <Tabs>
        {mode === QUARTZ && (
          <div data-tab="second" aria-current={tab === "second" || null} onClick={actions.tab}>
            Seconds
          </div>
        )}
        <div data-tab="minute" aria-current={tab === "minute" || null} onClick={actions.tab}>
          Minutes
        </div>
        <div data-tab="hour" aria-current={tab === "hour" || null} onClick={actions.tab}>
          Hours
        </div>
        <div data-tab="day" aria-current={tab === "day" || null} onClick={actions.tab}>
          Day
        </div>
        <div data-tab="month" aria-current={tab === "month" || null} onClick={actions.tab}>
          Month
        </div>
        <div data-tab="year" aria-current={tab === "year" || null} onClick={actions.tab}>
          Year
        </div>
      </Tabs>
      ...
      <Options>
        ...
        {tab === "day" ? (
          <>
            ...
            >
              Every{" "}
              <select data-type="w/" value={gen[tab]["w/"][0]} onChange={actions.arg0}>
                {allValuesFromOne}
              </select>{" "}
              {tab}(s) starting on{" "}
              <select data-type="w/" value={gen[tab]["w/"][1]} onChange={actions.arg1}>
            ...
            >
              Every{" "}
              <select data-type="m/" value={gen[tab]["m/"][0]} onChange={actions.arg0}>
                {domAllValuesFromOne}
              </select>{" "}
              {tab}(s) starting on the{" "}
              <select data-type="m/" value={gen[tab]["m/"][1]} onChange={actions.arg1}>
            ...
            {mode === QUARTZ && (
              <>
                ...
                >
                  ...
                  <select data-type="mW" value={gen[tab]["mW"][0]} onChange={actions.arg0}>
                    {domNamedOptionsFromFirst}
                  </select>{" "}
                  of the month
                </RadioOption>
                ...
                >
                  On the last{" "}
                  <select data-type="wL" value={gen[tab]["wL"][0]} onChange={actions.arg0}>
                    {namedOptionsFromFirst}
                  </select>{" "}
                  of the month
                </RadioOption>
          ...
          <>
            ...
            >
              Every{" "}
              <select data-type="/" value={gen[tab]["/"][0]} onChange={actions.arg0}>
                {allValuesFromOne}
              </select>{" "}
              {tab}(s) starting at {tab}{" "}
              <select data-type="/" value={gen[tab]["/"][1]} onChange={actions.arg1}>
                {namedOptionsFromFirst}
              </select>
            </RadioOption>
