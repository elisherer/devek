const HEBREW_MONTHS = ["תשרי", "חשוון", "כסלו", "טבת", "שבט", "אדר א'", leap => leap ? "אדר ב'" : "אדר", "ניסן", "אייר", "סיוון", "תמוז", "אב", "אלול"];
const HEBREW_DAYS = ["'א'", "ב'", "ג'", "ד'", "ה'", "ו'", "ז'", "ח'", "ט'", "י'", "י״א", "י״ב", "י״ג", "י״ד", "ט״ו", "ט״ז", "י״ז", "י״ח", "י״ט", "כ'", "כ״א", "כ״ב", "כ״ג", "כ״ד", "כ״ה", "כ״ו", "כ״ז", "כ״ח", "כ״ט", "ל"];
const GEMATRIA = { 1: 'א', 2: 'ב', 3: 'ג', 4: 'ד', 5: 'ה', 6: 'ו', 7: 'ז', 8: 'ח', 9: 'ט', 10: 'י', 20: 'כ', 30: 'ל', 40: 'מ', 50: 'נ', 60: 'ס', 70: 'ע', 80: 'פ', 90: 'צ', 100: 'ק', 200: 'ר', 300: 'ש', 400: 'ת' };
...
function convertToHebrewDate(date) {
  let days = Math.round((date - BEGINNING_OF_1900) / 864e5) + DAYS_UNTIL_1900, // days since 1/1/1900 + days from beginning until 1/1/1900
    d, m, y = Math.floor(days / AVERAGE_YEAR_LENGTH) + 1;
  ...
  if (isSameDate(alephBeTishrei, date)) {
    ...
    const lengthOfYear = Math.round((getAlephBeTishreiDate(y + 1) - getAlephBeTishreiDate(y)) / 864e5),
      isLeap = isLeapYear(y);
    ...
    while (!done) {
      let monthLength;
      if (m === 2) {
        monthLength = (/* Shalem */ lengthOfYear == 355 || lengthOfYear == 385 ? 30 : 29);
      } else if (m === 3) {
        monthLength = (/* Haser */ lengthOfYear == 353 || lengthOfYear == 383 ? 29 : 30);
...
function hebrewYearInGematria(hebYear) {
  ...
  return yearStr.substr(0, yearStr.length - 1) + '״' + yearStr[yearStr.length - 1];
}
...
export default date => {
  ...
  const month = typeof HEBREW_MONTHS[m - 1] === 'function' ? HEBREW_MONTHS[m - 1](isLeapYear(y)) : HEBREW_MONTHS[m - 1];
