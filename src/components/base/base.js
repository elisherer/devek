...
const radixRegex = Array.from(
  { length: 36 - 2 + 1 },
  (_, i) => new RegExp("^[-+]?([" + radixStr.substr(0, 2 + i) + "]+|Infinity)$"),
);
const parseIntStrict = (value, radix) => {
  if (radix >= 2 && radix <= 36 && radixRegex[radix - 2].test(value)) return parseInt(value, radix);
  return NaN;
};
...
const parsers = {
  ...
  binary: binary => (binary && !binaryRegex.test(binary) ? NaN : devek.binaryStringToArray(binary)),
  hex: hex => (hex && !hexRegex.test(hex) ? NaN : devek.hexStringToArray(hex)),
  base64: b64 => {
    try {
