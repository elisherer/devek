...
const Wrapper = styled.label`
  ...
  background: ${({ theme, readOnly, disabled }) =>
    readOnly || disabled ? theme.inputDisabledBackground : theme.inputBackground};
...
const ListBox = (
  { className, size, options, readOnly, value, disabled, numbered, indexed, hidePopup, uid, ...more } /*: {
  className?: string,
  size?: Number,
  options: Array,
  readOnly?: boolean,
  ...
  return (
    <Wrapper className={className}>
      <select readOnly={readOnly} size={size || 3} value={value} onKeyDown={blurOnEscape} disabled={disabled} {...more}>
        {options &&
          options.map((x, i) => (
