...
const Ul = styled.ul`
	...
	border-top: ${({ inline, theme }) =>
		inline ? "1px solid " + theme.greyBorder : "none"};
...
const ChecklistBox = ({
	...
	const memoLabel = useMemo(
		() => {
			...
			const mappedValue = value.map(x => {
				...
				return Object.prototype.hasOwnProperty.call(found, "name")
					? found.name
					: found;
			});
	...
	const handleChange = useCallback(
		e => {
			const availableValues =
				value &&
				options.map(x =>
					Object.prototype.hasOwnProperty.call(x, "value") ? x.value : x
				);
	...
	const ul = (
		<Ul open={inline || open} inline={inline}>
			{options &&
				options.map(x => {
					const localValue = Object.prototype.hasOwnProperty.call(x, "value")
						? x.value
						: x;
					return (
						<li key={localValue}>
