class Uint64 {
	...
	shl(dist) {
		for (let i = 0; i < dist; i++) {
			...
			if ((this.low & 0x80000000) !== 0) this.high |= 0x01;
			this.low = this.low << 1;
		}
		return this;
	}
	...
	shr(dist) {
		for (let i = 0; i < dist; i++) {
			this.low = this.low >>> 1;
			if ((this.high & 1) !== 0) this.low |= 0x80000000;
			this.high = this.high >>> 1;
		}
		return this;
	}
	...
	reflect() {
		let newHighVal = 0,
			newLowVal = 0;
		for (let i = 0; i < 32; i++) {
			if ((this.high & (1 << (31 - i))) !== 0) newLowVal |= 1 << i;
			if ((this.low & (1 << i)) !== 0) newHighVal |= 1 << (31 - i);
		}
	...
	toString(radix) {
		if (radix === 2)
			return (
				(this.high >>> 0).toString(2) +
				(this.low >>> 0).toString(2).padStart(32, "0").slice(-32)
			);
		if (!radix || radix === 16)
			return (
				(this.high >>> 0).toString(16) +
				(this.low >>> 0).toString(16).padStart(8, "0").slice(-8)
			);
