...
export default (
	...
	const store = {
		dispatch: () => {
			throw new Error("Don't call actions before calling useStore");
		}
	};
	...
	store.actions = Object.keys(actionCreators).reduce((a, type) => {
		a[type] = function () {
			const payload = actionCreators[type].apply(this, arguments)(
				store.state,
				store.actions
			);
			const dispatch = payload => {
				if (typeof payload !== "undefined") store.dispatch({ type, payload });
			};
