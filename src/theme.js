...
const base = {
	// constants
	nativeFont:
		'16px -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Oxygen, Ubuntu, Cantarell, "Fira Sans", "Droid Sans", "Helvetica Neue", sans-serif, "Apple Color Emoji", "Segoe UI Emoji", "Segoe UI Symbol"',
	fontMono:
	...
	togglebarActiveHoverBackground: x =>
		hexLighten(x.secondaryColor, x.dark ? -10 : 10),
	togglebarHoverBackground: x =>
		hexLighten(x.secondaryColor, x.dark ? -30 : 30),
	...
	checkeredShade: "#eee",
	checkeredBackground: x =>
		`linear-gradient(45deg, ${x.checkeredShade} 25%, transparent 25%, transparent 75%, ${x.checkeredShade} 75%, ${x.checkeredShade} 100%), linear-gradient(45deg, ${x.checkeredShade} 25%, ${x.checkeredLight} 25%, ${x.checkeredLight} 75%, ${x.checkeredShade} 75%, ${x.checkeredShade} 100%)`,

	navBackground: x =>
		`linear-gradient(130deg, ${x.primaryColor}, ${hexDarken(
			x.primaryColor,
			20
		)})`,
